...
/*
 * sample.c : Example MEX-file code emulating the functionality of the
 *             MATLAB command EVAL
 *
 *              
	     ...
	     int nrhs, const mxArray *prhs[] )
{
  ...
  if(nrhs==0)
    mexErrMsgIdAndTxt( "MATLAB:sample:minrhs",
            "Function 'sample' not defined for variables of class 'double'.\n");

  else if(!mxIsChar(prhs[0])) {
    const char str[]="Function 'sample' not defined for variables of class";
    char errMsg[100];
    
    sprintf(errMsg,"%s '%s'\n",str,mxGetClassName(prhs[0]));
    mexErrMsgIdAndTxt( "MATLAB:sample:UndefinedFunction", errMsg);
  }
